...
public class PendulumRunner {
		...
		GravityModel g = new GravityConstant(GravityConstant.EARTH_GRAVITY);
		...
		for (int second = 1; second <= 20; second++) {
			...
			if (second == 10) {
				g = new GravityConstant(GravityConstant.JUPITER_GRAVITY);
				rp.setGravityModel(g);
				rpCoarse.setGravityModel(g);
				sp.setGravityModel(g);
			}
		}
	}
}
